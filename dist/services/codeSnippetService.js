"use strict";
// src/services/codeSnippetService.ts
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.createCodeSnippet = void 0;
const codeSnippetModel_1 = __importDefault(require("../models/codeSnippetModel"));
const createCodeSnippet = (_a) => __awaiter(void 0, [_a], void 0, function* ({ problemId, language, starterCode, codeExample, }) {
    const newCodeSnippet = yield codeSnippetModel_1.default.create({
        problemId,
        language,
        starterCode,
        codeExample,
    });
    return newCodeSnippet;
});
exports.createCodeSnippet = createCodeSnippet;
//# sourceMappingURL=codeSnippetService.js.map